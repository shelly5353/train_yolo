...
function App() {
  ...
  // Auto-save and navigate helper
  const navigateToImage = useCallback(async (newIndex: number) => {
    if (isNavigating || newIndex < 0 || newIndex >= images.length || newIndex === currentImageIndex) {
      return;
    }
  ...
  }, [isNavigating, images, currentImageIndex, annotations]);

  // Navigation
  const goToPrevious = useCallback(() => {
    if (currentImageIndex > 0 && !isNavigating) {
      navigateToImage(currentImageIndex - 1);
    }
  }, [currentImageIndex, isNavigating, navigateToImage]);

  const goToNext = useCallback(() => {
    if (currentImageIndex < images.length - 1 && !isNavigating) {
      navigateToImage(currentImageIndex + 1);
    }
  }, [currentImageIndex, images.length, isNavigating, navigateToImage]);

  const selectImage = useCallback((index: number) => {
    if (!isNavigating && index >= 0 && index < images.length && index !== currentImageIndex) {
      navigateToImage(index);
    }
  }, [isNavigating, images.length, currentImageIndex, navigateToImage]);
        ...
        <div className="h-12 bg-white border-b border-gray-200 flex items-center px-4">
          ...
          {(isSaving || isNavigating) && (
            <div className="ml-auto flex items-center gap-2 text-blue-600">
              ...
              <span className="text-sm">{isSaving ? 'Saving...' : 'Loading...'}</span>
            </div>
          )}
        </div>
